...
for (const boundary of ['open2d', 'pbc', 'cylinder']) {
  ...
  const result = spawnSync('node', ['research/selfplay.mjs', '--game', '2dgo', '--boundary', boundary, '--lattice', 'square', '--botA', 'builtin', '--botB', 'builtin', '--games', games, '--out', out], { stdio: 'inherit' });
