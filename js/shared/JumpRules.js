...
export function createDirectionVectors(dimension) {
...
export function createHomeTargetZones({ dimension = 2, size = 8, topology = 'plane', targetAxis = 'x', zoneMode = 'auto', labTargetMode = 'opponentHome' } = {}) {
  ...
  const closed = ['torus', 'pbc', 't3', '4d-torus', 'torus4d', 'mobius', 'klein', 'klein_bottle', 'rp2', 'projective', 'projection'].includes(top);
...
export class JumpGameState {
  constructor(options = {}) {
    ...
    this.directions = createDirectionVectors(this.dimension);
...
function scoreJumpMove(game, move, player) {
  ...
  const center = targetCenter(target, game.dimension);
  const before = distanceChebyshev(move.from, center);
  const after = distanceChebyshev(move.to, center);
  const jumpBonus = move.type === 'jump' ? 6 : 0;
  const targetBonus = target.has(coordKey(move.to)) ? 20 : 0;
  return (before - after) * 5 + jumpBonus + targetBonus + Math.random() * 0.1;
}
