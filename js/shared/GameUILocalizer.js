const ZH = new Map(Object.entries({
  ...
  'Topologies: flat, torus, Klein, Mobius, RP2, S2, R3, 4D where supported.': '拓撲：在支援時可使用 flat、torus、Klein、Mobius、RP2、S2、R3、4D。',
  'Pauli-labeled local operators on topology-aware graph rays with Clifford frame transforms.': '在感知拓撲的圖射線上使用 Pauli 標記局部算符，並套用 Clifford frame 變換。',
  ...
  'Empty, Black/White controlled Pauli operators I/X/Y/Z with optional sign and phase.': '空格點，以及由 Black/White 控制、可帶符號與相位的 Pauli operators I/X/Y/Z。',
  'symbolic Pauli label plus owner sector': '符號 Pauli label 加上 owner sector',
  'Legal updates use graph rays, seam transport, and local Pauli-frame compatibility.': '合法更新使用 graph rays、seam transport 與局部 Pauli-frame 相容性。',
  'A discrete operator sandbox for topology-dependent Pauli transport.': '用於拓撲相依 Pauli transport 的離散 operator sandbox。',
  'Topology-aware Clifford local update': '拓撲感知 Clifford 局部更新',
  'Graph rays and adjacent vertices from the selected topology.': '來自所選拓撲的 graph rays 與相鄰 vertices。',
  ...
  'Educational Pauli/Clifford operator transport on finite graphs.': '有限圖上的 Pauli/Clifford operator transport 教學實驗。',
  'Toy symbolic algebra; not a calibrated quantum device simulation.': 'Toy symbolic algebra；不是校準過的量子裝置模擬。',
  'Internal consistency and seeded replay checks are the intended baseline.': '內部一致性與帶 seed 的 replay 檢查是目前基準。',
  'Empty graph vertices and inserted X/Y/Z Pauli operators.': '空 graph vertices 與插入的 X/Y/Z Pauli operators。',
  'symbolic site state': '符號 site state',
  'Insertion and local interactions follow graph adjacency.': '插入與局部互動依照 graph adjacency。',
  'Operator insertion field for discrete Pauli labels.': '用於離散 Pauli labels 的 operator insertion field。',
  'Pauli insertion field update': 'Pauli insertion field 更新',
  'Local graph adjacency and topology seams.': '局部 graph adjacency 與 topology seams。',
  'Inserted operators': '已插入 operators',
  'Label distribution': 'Label 分布',
  'Topology contacts': 'Topology contacts',
  'Graph insertion model for operator-label exploration.': '用於探索 operator label 的 graph insertion model。',
  'Toy symbolic estimator.': 'Toy symbolic estimator。',
  'Requires benchmark adapters before research-grade claims.': '提出 research-grade 主張前，需要 benchmark adapters。',
  'Mobile X/Y/Z operator tokens and empty graph vertices.': '可移動 X/Y/Z operator tokens 與空 graph vertices。',
  'token state plus symbolic Pauli label': 'token state 加上符號 Pauli label',
  'Movement follows graph adjacency and selected exchange paths.': '移動依照 graph adjacency 與所選 exchange paths。',
  'Worldline transport sandbox for local operator labels.': '用於局部 operator labels 的 worldline transport sandbox。',
  'Pauli worldline hop/exchange': 'Pauli worldline 躍遷 / 交換',
  'Adjacent vertices and jump paths in the topology graph.': 'Topology graph 中的相鄰 vertices 與 jump paths。',
  'Token counts': 'Token 計數',
  'Path history': 'Path history',
  'Pauli labels': 'Pauli labels',
  'Exchange events': 'Exchange events',
  'Worldline visualization for topology-dependent label transport.': '用於拓撲相依 label transport 的 worldline 視覺化。',
  'Toy transport model.': 'Toy transport model。',
  'Replay and event-log consistency only.': '僅提供 replay 與 event log 一致性檢查。',
  'I/X/Y/Z qubit-site operators with sign, phase, owner sector, and optional ancilla state.': '具有符號、相位、owner sector 與可選 ancilla state 的 I/X/Y/Z qubit-site operators。',
  ...
  'Discrete stabilizer recovery estimator on a reusable topology graph.': '可重用 topology graph 上的離散 stabilizer recovery estimator。',
  'Pauli-frame recovery update': 'Pauli-frame recovery 更新',
  'Local graph neighborhoods and topology cycles.': '局部 graph neighborhoods 與 topology cycles。',
  'Manual recovery actions with optional measurement/noise events': '手動 recovery actions，可選 measurement / noise events',
  'Seeded stochastic when measurement error or noise is enabled.': '啟用 measurement error 或 noise 時為帶 seed 的隨機過程。',
  'Syndrome weight': 'Syndrome 權重',
  'Logical sector': 'Logical sector',
  'Global parity': 'Global parity',
  'Vacuum recovery': 'Vacuum recovery',
  'Stabilizer bookkeeping is discrete and graph-local.': 'Stabilizer bookkeeping 是離散且 graph-local 的。',
  'Estimator for recovery behavior, not hardware-calibrated QEC.': '這是 recovery behavior estimator，不是 hardware-calibrated QEC。',
  'Benchmarks should compare against known stabilizer examples before upgrade.': '升級前應與已知 stabilizer examples 比較 benchmark。',
  'Vacuum and anyon charge labels from the selected anyon model.': 'Vacuum 與所選 anyon model 的 anyon charge labels。',
  'symbolic charge labels': '符號 charge labels',
  'Fusion rules and topology-aware local contacts.': 'Fusion rules 與 topology-aware local contacts。',
  'Discrete topological charge fusion sandbox.': '離散 topological charge fusion sandbox。',
  'Local charge fusion update': '局部 charge fusion 更新',
  'Adjacent graph sites and local interval contacts.': '相鄰 graph sites 與 local interval contacts。',
  'Charge counts': 'Charge 計數',
  'Fusion outcomes': 'Fusion outcomes',
  'Symbolic anyon fusion rules on finite graphs.': '有限圖上的符號 anyon fusion rules。',
  'Toy topological matter visualization.': 'Toy topological matter 視覺化。',
  'Requires external benchmark examples for stronger claims.': '提出更強主張前，需要外部 benchmark examples。',
  'Mobile anyon tokens, vacuum vertices, braid words, fusion channels, and energy metadata.': '可移動 anyon tokens、vacuum vertices、braid words、fusion channels 與 energy metadata。',
  'token state plus symbolic charge and braid memory': 'token state 加上符號 charge 與 braid memory',
  'Graph-local hops, exchanges, inverse unbraiding, and model-specific fusion rules.': 'Graph-local hops、exchanges、inverse unbraiding 與模型專屬 fusion rules。',
  'Worldline workspace for topological charge transport.': '用於 topological charge transport 的 worldline workspace。',
  'Anyon hop, braid, unbraid, and fusion update': 'Anyon hop、braid、unbraid 與 fusion 更新',
  'Adjacent vertices, jump paths, and topology cycles.': '相鄰 vertices、jump paths 與 topology cycles。',
  'Seeded stochastic when anyon noise is enabled.': '啟用 anyon noise 時為帶 seed 的隨機過程。',
  'Total fusion charge': 'Total fusion charge',
  'Braid word length': 'Braid word length',
  'Energy history': 'Energy history',
  'Unbraid success': 'Unbraid success',
  'Symbolic braid/fusion memory with graph-local moves.': '具有 graph-local moves 的符號 braid / fusion memory。',
  'Toy or estimator depending on model and settings.': '依模型與設定屬於 toy 或 estimator。',
  'Replay and known fusion-rule checks are the baseline.': 'Replay 與已知 fusion-rule checks 是目前基準。',
  'Identity background and primary-field insertions such as sigma and epsilon.': 'Identity background，以及 sigma、epsilon 等 primary-field insertions。',
  'symbolic primary field with channel metadata': '帶 channel metadata 的符號 primary field',
  'Graph contact, OPE channel rules, and optional Virasoro truncation.': 'Graph contact、OPE channel rules 與可選 Virasoro truncation。',
  'Graph estimator for field insertions and CFT-like observables.': '用於 field insertions 與 CFT-like observables 的 graph estimator。',
  'Primary insertion and Virasoro graph update': 'Primary insertion 與 Virasoro graph 更新',
  'Local graph contacts, intervals, and selected generator direction.': 'Local graph contacts、intervals 與所選 generator direction。',
  'Manual insertion, generator, and measurement events': '手動 insertion、generator 與 measurement events',
  'Seeded stochastic only when measurement/noise options are enabled.': '僅在啟用 measurement / noise options 時為帶 seed 的隨機過程。',
  'Dominant conformal block': '主導 conformal block',
  'OPE channels': 'OPE channels',
  'Entropy estimate': 'Entropy estimate',
  'Correlation estimate': 'Correlation estimate',
  'Stress proxy': 'Stress proxy',
  'Discrete graph estimators approximate selected CFT concepts.': '離散 graph estimators 近似部分 CFT concepts。',
  'Estimator, not an exact continuum CFT calculation.': '這是 estimator，不是精確 continuum CFT calculation。',
  'Known limitations should remain visible with every export.': '每次匯出都應保留已知限制。',
  'Spin up s=+1, spin down s=-1, and optional empty/undecided graph sites.': 'Spin up s=+1、spin down s=-1，以及可選 empty / undecided graph sites。',
  'integer spin sector per site': '每個 site 的整數 spin sector',
  'Graph-neighbor interactions and optional Metropolis acceptance.': 'Graph-neighbor interactions 與可選 Metropolis acceptance。',
  'Discrete spin field on a topology-aware lattice graph.': 'Topology-aware lattice graph 上的離散 spin field。',
  'Ising graph energy and domain-wall update': 'Ising graph energy 與 domain-wall 更新',
  'Nearest graph neighbors from the selected topology.': '來自所選拓撲的 nearest graph neighbors。',
  'Manual local update with optional Metropolis stochastic acceptance': '手動局部更新，可選 Metropolis stochastic acceptance',
  'Seeded stochastic when temperature/Metropolis or noise is enabled.': '啟用 temperature / Metropolis 或 noise 時為帶 seed 的隨機過程。',
  'Energy': 'Energy',
  'Magnetization': 'Magnetization',
  'Domain wall length': 'Domain wall length',
  'Domain count': 'Domain count',
  'Topology winding': 'Topology winding',
  'Finite graph Ising-like estimator with selected boundary conditions.': '具所選 boundary conditions 的 finite graph Ising-like estimator。',
  'Discrete estimator; not a continuum thermodynamic solver.': '離散 estimator；不是 continuum thermodynamic solver。',
  'Benchmarking should compare energy and magnetization against known Ising cases.': 'Benchmarking 應與已知 Ising cases 的 energy 與 magnetization 比較。',
  'Phase A, phase B, and metastable empty substrate.': 'Phase A、phase B 與 metastable empty substrate。',
  'enum site state': 'enum site state',
  'Growth and interface updates follow graph contacts and energy settings.': 'Growth 與 interface updates 依照 graph contacts 與 energy settings。',
  'Discrete phase-field competition on a topology graph.': 'Topology graph 上的離散 phase-field competition。',
  'Nucleation, growth, and interface update': 'Nucleation、growth 與 interface 更新',
  'Adjacent graph sites and interface contacts.': '相鄰 graph sites 與 interface contacts。',
  'Manual or auto-selected local update': '手動或自動選擇的局部更新',
  'Seeded stochastic when droplet noise is enabled.': '啟用 droplet noise 時為帶 seed 的隨機過程。',
  'Interface length': 'Interface length',
  'Area fractions': 'Area fractions',
  'Wrapped interfaces': 'Wrapped interfaces',
  'Toy phase-field energy on a finite graph substrate.': '有限 graph substrate 上的 toy phase-field energy。',
  'Estimator for qualitative interface behavior.': '用於定性 interface behavior 的 estimator。',
  'Needs benchmark cases before quantitative use.': '定量使用前需要 benchmark cases。',
  'Empty resource sites and occupied cluster sectors A/B.': 'Empty resource sites 與 occupied cluster sectors A/B。',
  'enum site state with connected-component metadata': '帶 connected-component metadata 的 enum site state',
  'Connected clusters require graph-neighbor contacts; zero-contact clusters can be removed.': 'Connected clusters 需要 graph-neighbor contacts；zero-contact clusters 可以被移除。',
  'Statistical cluster dynamics on finite topology graphs.': '有限 topology graphs 上的 statistical cluster dynamics。',
  'Cluster placement, growth, and zero-contact removal': 'Cluster placement、growth 與 zero-contact removal',
  'Connected graph clusters and nearest-neighbor resource contacts.': 'Connected graph clusters 與 nearest-neighbor resource contacts。',
  'Manual or auto local update with optional diffusion/noise': '手動或自動局部更新，可選 diffusion / noise',
  'Seeded stochastic when cluster noise is enabled.': '啟用 cluster noise 時為帶 seed 的隨機過程。',
  'Largest cluster': 'Largest cluster',
  'Cluster-size distribution': 'Cluster-size distribution',
  'Percolation probability': 'Percolation probability',
  'Wrapping clusters': 'Wrapping clusters',
  'Statistical dynamics interpretation only; biological species language belongs in Topoboard Life.': '僅作 statistical dynamics 解釋；生物 species 語言屬於 Topoboard Life。',
  'Estimator for graph percolation and cluster growth.': '用於 graph percolation 與 cluster growth 的 estimator。',
  'Percolation benchmarks should be added for known lattices.': '應加入已知 lattices 的 percolation benchmarks。',
  'Empty vertices and mobile particle species/charge signs.': 'Empty vertices 與可移動 particle species / charge signs。',
  'token or site occupancy state': 'token 或 site occupancy state',
  'Hops, exchanges, scattering paths, and recombination follow graph reachability.': 'Hops、exchanges、scattering paths 與 recombination 依照 graph reachability。',
  'Discrete particle transport and reaction sandbox.': '離散 particle transport 與 reaction sandbox。',
  'Particle hop, exchange, scatter, and recombine update': 'Particle hop、exchange、scatter 與 recombine 更新',
  'Adjacent vertices and allowed multi-step scattering paths.': '相鄰 vertices 與允許的 multi-step scattering paths。',
  'Particle count': 'Particle count',
  'Recombination count': 'Recombination count',
  'Path parity': 'Path parity',
  'Average path length': 'Average path length',
  'Qualitative graph reaction-diffusion system.': '定性 graph reaction-diffusion system。',
  'Toy dynamics, not calibrated chemistry.': 'Toy dynamics，不是校準過的 chemistry。',
  'Replay and graph-reachability checks only.': '僅提供 replay 與 graph-reachability checks。',
  'Edge arrows aligned or anti-aligned with graph orientation.': '與 graph orientation 同向或反向的 edge arrows。',
  'signed edge state': 'signed edge state',
  'Vertex ice-rule checks and allowed string/loop flips.': 'Vertex ice-rule checks 與允許的 string / loop flips。',
  'Discrete constrained edge system with monopole-like defects.': '具有 monopole-like defects 的離散 constrained edge system。',
  'Edge-arrow flip, string, loop, and monopole update': 'Edge-arrow flip、string、loop 與 monopole 更新',
  'Incident edges, vertex checks, paths, and cycles.': 'Incident edges、vertex checks、paths 與 cycles。',
  'Ice-rule violations': 'Ice-rule violations',
  'Monopole count': 'Monopole count',
  'String length': 'String length',
  'Loop winding': 'Loop winding',
  'Finite graph vertex-model estimator.': 'Finite graph vertex-model estimator。',
  'Estimator for constrained edge dynamics.': '用於 constrained edge dynamics 的 estimator。',
  'Compare small graph sectors before stronger claims.': '提出更強主張前，請先比較 small graph sectors。',
  'Edge field Ue=+1 or Ue=-1 with optional charge/flux syndrome metadata.': 'Edge field Ue=+1 或 Ue=-1，並可帶 charge / flux syndrome metadata。',
  'Star checks, plaquette/cycle checks, and topology-cycle logical sectors.': 'Star checks、plaquette / cycle checks 與 topology-cycle logical sectors。',
  'Discrete Z2 lattice gauge and memory estimator.': '離散 Z2 lattice gauge 與 memory estimator。',
  'Edge, path, loop, check, and decoder update': 'Edge、path、loop、check 與 decoder 更新',
  'Incident edges, open paths, closed loops, local faces, and cycles.': 'Incident edges、open paths、closed loops、local faces 與 cycles。',
  'Manual event-driven update with optional decoder/noise': '手動事件驅動更新，可選 decoder / noise',
  'Seeded stochastic when noisy edge flips are enabled.': '啟用 noisy edge flips 時為帶 seed 的隨機過程。',
  'Star violations': 'Star violations',
  'Plaquette flux': 'Plaquette flux',
  'Wilson loops': 'Wilson loops',
  'Discrete Z2 gauge toy/estimator on graph topology.': 'Graph topology 上的離散 Z2 gauge toy / estimator。',
  'Estimator; face definitions depend on available graph cell structure.': 'Estimator；face definitions 取決於可用的 graph cell structure。',
  'Small-code and known-loop tests should be benchmarked.': '應對 small-code 與 known-loop tests 做 benchmark。',
...
function translateValue(text) {
  ...
  return text
    .replace(/Standard boundary: rays stop at the edge\./g, 'Standard boundary：rays 在邊界停止。')
    .replace(/2D RBC: each boundary exit maps to one fixed random boundary square for this game\./g, '2D RBC：每個 boundary exit 都會映射到本局固定的一個 random boundary square。')
    .replace(/Cylinder: x wraps periodically while y remains open\./g, 'Cylinder：x 方向週期包回，y 方向保持開放。')
    .replace(/S2 latitude-ring graph: longitude wraps and the first\/last latitude rings connect to playable north\/south pole nodes\./g, 'S2 latitude-ring graph：longitude 方向包回，第一圈與最後一圈 latitude rings 連到可操作的 north / south pole nodes。')
    .replace(/Torus: x and y wrap periodically\./g, 'Torus：x 與 y 方向週期包回。')
