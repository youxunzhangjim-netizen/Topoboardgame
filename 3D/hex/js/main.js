...
function projectCoordinate(coordinate, width, height) {
    ...
    const rotated = rotatePoint(coordinate.map((value) => value - midpoint));
    const scale = Math.min(width, height) * 0.62 / Math.max(1, size - 1) * Number(elements.zoom.value);
...
function drawBoard() {
    ...
    for (const coordinate of visible) {
        ...
        for (let axis = 0; axis < 3; axis += 1) {
            ...
            next[axis] += 1;
            if (next[axis] >= size || !visibleKeys.has(next.join(','))) continue;
    ...
    for (const site of projectedSites) {
        ...
        const isBlackTarget = site.coordinate[0] === 0 || site.coordinate[0] === xEnd;
        const isWhiteTarget = site.coordinate[1] === 0 || site.coordinate[1] === yEnd;
        ...
        context.fill();
        context.strokeStyle = color === HEX_COLORS.WHITE ? '#727b80' : 'rgba(228,239,245,0.7)';
