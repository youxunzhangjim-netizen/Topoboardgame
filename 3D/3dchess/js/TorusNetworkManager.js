...
export class TorusNetworkManager {
    constructor(game) {
        this.game = game;
        this.peer = null;
    ...
    findMatch() {
        ...
        this.matchmakingIndex = 0;
        this.game.gameMode = 'online';
        this.game.myColor = null;
        this.setStatus('connecting', 'online.matchmakingSearch');
        this.game.setStatus('online.matchmakingSearch');
        this.game.updateUI();
        this.tryMatchSlot(0);
    }
    ...
    tryMatchSlot(index = 0) {
        ...
        if (index >= slots.length) {
            this.matchmakingIndex = 0;
            this.setStatus('connecting', 'online.matchmakingRetry');
            this.game.setStatus('online.matchmakingRetry');
        ...
        peer.on('open', (id) => {
            ...
            this.roomId = id;
            this.setRoomInfo(id, '');
            this.setStatus('connecting', 'online.matchmakingWaiting');
            this.game.setStatus('online.matchmakingWaiting');
            this.game.updateUI();
            this.matchmakingTimer = window.setTimeout(() => {
                if (this.peer === peer && this.isMatchmaking && !this.isConnected) {
                    this.retryMatchmaking(index + 1, 'online.matchmakingTimeout');
                }
    ...
    joinMatchSlot(roomId, index) {
        ...
        peer.on('open', () => {
            ...
            this.startMatchConnectionTimer(index + 1);
        });
    ...
    retryMatchmaking(nextIndex = this.matchmakingIndex + 1, statusKey = 'online.matchmakingRetry') {
        if (!this.isMatchmaking) return;
        ...
        this.close({ silent: true, stopMatchmaking: false });
        this.isMatchmaking = true;
        this.setStatus('connecting', statusKey);
        this.game.setStatus(statusKey);
    ...
    matchmakingRoomIds() {
        ...
        const settingsKey = this.matchSettingsKey();
        const roomIds = [];
        for (const currentBucket of buckets) {
            for (let slot = 0; slot < MATCHMAKING_SLOT_COUNT; slot++) {
                roomIds.push(`tc-${settingsKey}-m-${currentBucket}-${slot}`);
            }
        }
        return roomIds;
    }
    ...
    matchSettingsKey() {
        return [
            this.safeSetting(this.variantKey()),
            this.safeSetting(this.game.boundaryCondition || 'standard'),
            `t${this.matchTimerValue()}`
        ].join('-');
    }

    matchTimerValue() {
        const value = Number(this.game.timeLimit);
        return Number.isFinite(value) && value > 0 ? Math.floor(value) : 0;
    }

    safeSetting(value) {
        return String(value || 'x').toLowerCase().replace(/[^a-z0-9]+/g, '').slice(0, 24) || 'x';
    }

    startMatchConnectionTimer(nextIndex) {
        this.clearConnectionTimer();
        this.connectionTimer = window.setTimeout(() => {
            if (this.isConnected) return;
            this.retryMatchmaking(nextIndex, 'online.matchmakingTimeout');
    ...
    joinRoom(rawRoomId, { color = 'black', reconnect = false } = {}) {
        ...
        peer.on('open', () => {
            ...
            this.connection = peer.connect(roomId, {
                serialization: 'json',
                metadata: { game: this.gameKey(), roomId, color, reconnect }
            });
            this.setupConnection(this.connection);
    ...
    async handleRemoteMessage(data) {
        ...
        switch (data.type) {
            case 'ready':
                if (this.isHost) this.finishMatchmaking();
            ...
            case 'roomFull':
                if (this.isMatchmaking) {
                    this.retryMatchmaking(this.matchmakingIndex + 1, 'online.matchmakingRetry');
                    break;
                }
            ...
            case 'wrongGame':
                if (this.isMatchmaking) {
                    this.retryMatchmaking(this.matchmakingIndex + 1, 'online.wrongGameMatch');
                    break;
                }
                this.setStatus('disconnected', 'online.wrongGame');
                this.game.setStatus('online.wrongGame');
                this.close({ silent: true });
                this.clearRoomInfo();
                break;
    ...
    applySyncState(state, { persist = true } = {}) {
        ...
        this.game.boundaryCondition = 'periodic';
    ...
    handlePeerError(err) {
        ...
        if (this.isMatchmaking && this.shouldRetryMatchmakingError(err)) {
            this.retryMatchmaking(this.matchmakingIndex + 1, 'online.matchmakingRetry');
            return;
        }
    ...
    shouldRetryMatchmakingError(err) {
        return ['peer-unavailable', 'network', 'socket-error', 'socket-closed', 'disconnected', 'server-error', 'webrtc'].includes(err?.type || '');
    }
    ...
    normalizeRoomCode(value) {
        ...
        const roomCode = text.match(/\b\d{5}\b/);
        if (roomCode) return roomCode[0];
