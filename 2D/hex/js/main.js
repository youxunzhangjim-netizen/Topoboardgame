...
function syncRobotControls() {
    if (elements.robotMove) elements.robotMove.disabled = !game || game.winner || elements.gameMode.value === 'online';
...
function scheduleRobot() {
    clearTimeout(robotTimer);
    syncRobotControls();
    if (elements.gameMode.value !== 'robot' || game.winner || game.currentColor !== robotSide()) return;
